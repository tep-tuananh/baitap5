...
public class Book {
    ...
    public Book(String bookID, String bookName, float inportPrice, float exportPrice, String author, float inerest, int year) {
        this.bookID = bookID;
    ...
    public void inputInfor(Scanner scanner, Book[] book, int index) {
    ...
    public void sapXepTangDan(Book[] book, int index) {// sắp xếp tăng dần theo giá sách nhập vào
    ...
    public void sapXepGiamDan(Book [] book , int index){ // sắp xêp sách theo lợi nhuận giảm dần
        for (int i = 0; i < index-1; i++) {
            for (int j = i + 1; j < index; j++) {
    ...
    public  void findBook(Scanner scanner,Book [] book , int index)
    {
    ...
    public  void thongKeTheoNam(Book [] book , int index)
    {
            int cout=0;
        for(int i=0;i<index-1;i++)
        {
